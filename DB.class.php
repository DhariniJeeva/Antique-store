...
require_once "/home/dj9875/db_conn.php";
...
class DB {
    ...
    //get DiscountedItems
    function get_discounted_items(){
        $getDiscountQuery = "SELECT COUNT(Saleprice) AS row from products where Saleprice!=0";
        if($stmt = $this->mysqli->prepare($getDiscountQuery)){
            //$stmt->bind_param("s",$password); //if four parameters then "ssss" - denotes strings
            ...
            if($num_rows>0){ //iterate over the results n print
                while($stmt->fetch()){
                        $row;

                }
            }
            //checking for discounted productss
            if($row>=4){
                $count =1;
            }
            else{
    ...
    function getUserID($username, $password){
        ...
        $data=array();
        $query = "SELECT userid FROM signup WHERE username=? AND password=?";
    ...
    function getAllPeople($username, $password){

        $query = "SELECT * FROM signup where username=? AND password=?";
        if($stmt = $this->mysqli->prepare($query)){
            $stmt->bind_param("ss", $username, $password); //if four parameters then "ssss" - denotes strings
    ...
    function uniqueCheckName($username){

        $query = "SELECT * FROM signup where username=?";
        if($stmt = $this->mysqli->prepare($query)){
            $stmt->bind_param("s",$username); //if four parameters then "ssss" - denotes strings
    ...
    function uniqueCheckEmail($Email){

        $query = "SELECT * FROM signup where Email=?";
        if($stmt = $this->mysqli->prepare($query)){
            $stmt->bind_param("s",$Email); //if four parameters then "ssss" - denotes strings
    ...
    function uniqueCheckProd($Name){

        $query = "SELECT * FROM products where Name=?";
        if($stmt = $this->mysqli->prepare($query)){
            $stmt->bind_param("s", $Name); //if four parameters then "ssss" - denotes strings
    ...
    //check for product in users cart
    function uniqueCheckProdUser($Name, $userid){

        $query = "SELECT * FROM cart where Name=? AND userid=?";
        if($stmt = $this->mysqli->prepare($query)){
            $stmt->bind_param("si", $Name, $userid); //if four parameters then "ssss" - denotes strings
